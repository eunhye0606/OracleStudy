...
*/
-- SYS에서 SCOTT계정 만들때 DEFUALT TABLESPACE가 USERS였다.
-- 즉, TABLESPACE를 따로 지정하지 않으면 DEFUALT TABLESPACE로 
-- 테이블이 생성됨을 확인!
-- 테이블스페이스에 접근 권한이 있으면 그 테이블스페이스에 테이블생성가능
-------------------------------------------------------------------------------
--★가장 중요한 부분★--
...
cf ) MSSQL 서버의 정수 표현 타입
     ...
     MSSQL 서버의 실수 표현 타입
     float, real(java의 double이라 생각하면 된다.)
...
/*
※ ORACLE 은 숫자 표현 타입이 한 가지로 통일 되어 있다.

   1. 숫자형 NUMBER        → -10의 38승-1 ~ 10의 38승(범위가 엄청 큼!)
             NUMBER(3)     → -999 ~ 999(3자리의 정수형)
             NUMBER(4)     → -9999 ~ 9999
             NUMBER(4, 1)  → -999.9 ~ 999.9(총 4자리인데 실수영역에 1자리를 빼줌)
...
※ ORACLE의 문자 표현 타입
   ...
   2. 문자형 CHAR        → 고정형 크기
             ...
             CHAR(10)    ← '곧잔다김민성'(10byte 를 초과하기 때문에 입려 발생)

             VARCHAR2    → 가변형 크기
             VARCHAR2(10)→ 담긴 데이터에 따라 크기가 변화
             VARCHAR2(10)← '김정용'(크기는 10byte로 지정했지만 6byte)
             ...
             NCHAR       → 유니코드기반 고정형 크기
                            ------------
                               글자수
             NCHAR(10)   → 10글자(한/영 똑같음 즉, byte가 아님.)
